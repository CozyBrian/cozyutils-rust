...
use crate::utils::fs::{
  component_template, ensure_dir, join_path, make_component_name, move_file,
  read_dir_and_sort, read_to_string, write_string,
};
...
pub fn svg_to_tsx(args: Vec<String>) -> Result<(), String> {
  ...
  if parsed.options.get("help").is_some() {
    println!(
      "Usage: -svg2tsx <directory> [--ext=.svg] [--dry-run] [--force] [--no-move]"
    );
    return Ok(());
  }
  ...
  let dashed_attribute_regex = Regex::new(r"(\w+)-(\w+)")
  ...
  for filename in &files {
    ...
    content = fill_regex.replace_all(&content, "fill=\"currentColor\"").to_string();
    ...
    if output_path.exists() && !force {
      println!(
        "File {}.tsx already exists. Skipping...",
        component_name
      );
      continue;
    }
  ...
  if !dry_run {
    any_to_export(vec![".tsx"], vec![directory.clone(), "index.ts".to_string()])?;
  }
